...
def train_agent(num_episodes, agent, env, batch_size, buffer_size):
    ...
    for episode in range(num_episodes):
            ...
            if done:
                if delay != float('inf') and area != float('inf'):
                    ...
                    if episode_adp < current_best_adp:
                        ...
                        best_selected_cells = list(np.where(env.mask == 1)[0])
        ...
        print(f"Episode {episode + 1}, Highest Reward = {highest_reward:.4f}, Epsilon = {epsilon:.6f}")
        epsilon *= 0.9995  # Decay epsilon

    return adp_history, best_adp_history, best_selected_cells
...
start = time.time()
adp_history, best_adp_history, best_selected_cells = train_agent(num_episodes, agent, env, batch_size, buffer_size)
end = time.time()
runtime = end - start
print('Total time: ', runtime)

# ==========================================
# Output Best Cells Selected
# ==========================================
print("\n>> Best Cells Selected for Minimum ADP:")
print(f"Selected Cell Indices: {best_selected_cells}")
if best_selected_cells:
    with open(genlib_origin, 'r') as f:
        all_gates = [line.strip() for line in f if line.startswith("GATE") and not line.startswith("GATE BUF") and not line.startswith("GATE INV") and not line.startswith("GATE sky130_fd_sc_hd__buf") and not line.startswith("GATE sky130_fd_sc_hd__inv") and not line.startswith("GATE gf180mcu_fd_sc_mcu7t5v0__buf") and not line.startswith("GATE gf180mcu_fd_sc_mcu7t5v0__inv")]
    selected_gate_names = [all_gates[i].split()[1] if len(all_gates[i].split()) > 1 else all_gates[i] for i in best_selected_cells]
    print(f"Selected Gate Names: {selected_gate_names}")
    
    # Write to file
    os.makedirs("records", exist_ok=True)
    with open("records/new_batched_ddqn_cells.txt", 'w') as f:
        f.write(f"Design: {design}\n")
        f.write(f"Best ADP: {best_adp_history[-1]:.2f}\n")
        f.write(f"Number of Selected Gates: {len(best_selected_cells)}\n\n")
        f.write(f"Selected Cell Indices:\n{best_selected_cells}\n\n")
        f.write(f"Selected Gate Names:\n")
        for i, gate_name in enumerate(selected_gate_names):
            f.write(f"{i+1}. {gate_name}\n")
    print(f"\n>> Best cells saved to records/new_batched_ddqn_cells.txt")
print(f"Best ADP Achieved: {best_adp_history[-1]:.2f}\n")
...
plt.savefig(f"records/new_batched_ddqn_adp_{num_episodes}.png", dpi=300)
print(f">> Visualization successfully saved to records/new_batched_ddqn_adp_{num_episodes}.png")