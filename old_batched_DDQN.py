...
def train_agent(num_episodes, agent, env, batch_size, buffer_size):
    ...
    for episode in range(num_episodes):
            ...
            if done:
                if delay != float('inf') and area != float('inf'):
                    ...
                    if episode_adp < current_best_adp:
                        ...
                        best_selected_cells = list(np.where(env.state == 1)[0])
        ...
        print(f"Episode {episode + 1}, Highest Reward = {highest_reward}, Epsilon = {epsilon:.6f}")
...
if best_selected_cells:
    ...
    with open("records/old_batched_ddqn_cells.txt", 'w') as f:
    ...
    print(f"\n>> Best cells saved to records/old_batched_ddqn_cells.txt")
...
plt.savefig(f"records/adp_old_batched_ddqn_{num_episodes}.png", dpi=300)
print(f">> Visualization successfully saved to records/old_batched_ddqn_adp_{num_episodes}.png")
