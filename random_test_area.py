...
plt.title(f"Random Selection Area Optimization\nDesign: {design}")
plt.xlabel("Training Episodes")
plt.ylabel("Area")
plt.legend()
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.text(0.8, 0.98, f'Training Time: {runtime:.2f}s\nBest Area: {clean_best_area[-1]:.2f}',
         transform=plt.gca().transAxes, fontsize=10, verticalalignment='top',
         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

output_path = f"random_test/random_{num_iterations}_{design.split('/')[1].split('.')[0]}_{sample_gate}_{lib_origin[:-4]}_area.png"
